...
class OutputChannel(object):
    ...
    def send_response(self, recipient_id: Text,
        ...
        if message.get("image"):#reached
            self.send_image_url(recipient_id, message.get("image"))

        if message.get("video"):
            self.send_video_url(recipient_id, message.get("video"))
    ...
    def send_video_url(self, recipient_id: Text, video_url: Text) -> None:
        """Sends a video. Default will just post the url as a string."""

        self.send_text_message(recipient_id, "Video: {}".format(video_url))
