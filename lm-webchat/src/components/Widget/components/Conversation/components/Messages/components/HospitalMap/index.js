...
const alldata = [
  ["LATITUDE", "LONGITUDE", "City", "Count"]
];
const topdata = [['City', 'Count']];
    ...
    axios
      ...
      .then((response) => {
        ...
        for (let i = 0; i < response["data"]["cityCountList"].length; i++) {
          // loop through your data response["data"]["cityCountList"].length
    ...
    axios
      ...
      .then((response) => {;
        for (let i = 0; i < response["data"]["cityCountList"].length; i++) {
          ...
          {alldata.push([parseFloat(response["data"]["cityCountList"][i]["latitude"]), parseFloat(response["data"]["cityCountList"][i]["longitude"]), response["data"]["cityCountList"][i]["geoCity"], parseFloat(response["data"]["cityCountList"][i]["rankCount"])])};
        }
      })
      .catch((error) => { console.log(error); });
  }
          ...
          chartEvents={[
            {
              ...
              callback: ({ chartWrapper }) => {
                ...
                const selection = chart.getSelection();
                if (selection.length === 0) return;
                const region = alldata[selection[0].row + 1];
                console.log("Selected : " + region);
              }
            }
          ]}
